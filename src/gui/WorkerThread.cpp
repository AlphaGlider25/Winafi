...
WorkerThread::WorkerThread(const QString &isoPath, const QString &devnode,
                          ...
                          int wueFlags, const QString &username, QObject *parent)
    : QThread(parent), m_isoPath(isoPath), m_devnode(devnode),
      ...
      m_wueFlags(wueFlags), m_username(username) {}
...
void WorkerThread::run() {
    ...
    if (m_wueFlags != 0) {
        winafi_session_set_unattend(session, m_wueFlags,
            m_username.isEmpty() ? nullptr : m_username.toUtf8().constData());
    }
