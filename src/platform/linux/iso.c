...
// Extract install.esd from ISO using 7z (handles UDF format that libarchive doesn't support)
static int iso_extract_esd_to_temp(const char *iso_path, char *out_temp_path, size_t path_size) {
    if (!iso_path || !out_temp_path) return -1;

    snprintf(out_temp_path, path_size, "/tmp/install.esd.tmp");

    // Check if 7z is available
    if (access("/usr/bin/7z", X_OK) != 0) {
        fprintf(stderr, "7z not found - cannot extract ESD format ISO\n");
        return -1;
    }

    // Use 7z to extract - it supports UDF format
    // Try both x64 and x86 paths (prefer x64)
    const char *esd_paths[] = { "x64/sources/install.esd", "x86/sources/install.esd" };
    for (int i = 0; i < 2; i++) {
        pid_t pid = fork();
        if (pid < 0) return -1;

        if (pid == 0) {
            // Child process: extract with 7z
            ...
            execl("/usr/bin/7z", "7z", "x", "-o/tmp", iso_path, esd_paths[i], (char*)NULL);
            exit(127);
        } else {
            ...
            if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
                // Successfully extracted - move to target location
                char source_path[4096];
                snprintf(source_path, sizeof(source_path), "/tmp/%s", esd_paths[i]);
                if (rename(source_path, out_temp_path) == 0) {
                    return 0;  // Success
                }
            }
        }
    ...
    return -1;  // Failed to extract ESD
}

// Apply WIM/ESD image to directory using wimapply
static int iso_apply_wim_esd(const char *wim_path, const char *target_dir) {
    if (!wim_path || !target_dir) return -1;

    // Check if wimapply is available
    if (access("/usr/bin/wimapply", X_OK) != 0) {
        fprintf(stderr, "wimapply not found - cannot extract ESD format ISO\n");
        return -1;  // E-30-A
    }

    // Build wimapply command: wimapply <wim> 1 <target>
    // Image 1 is typically the first (and often only) image in Windows WIM files
    pid_t pid = fork();
    if (pid < 0) return -1;

    if (pid == 0) {
        // Child process
        execl("/usr/bin/wimapply", "wimapply", wim_path, "1", target_dir, (char*)NULL);
        // If execl fails
        perror("wimapply");
        exit(127);
    } else {
        ...
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
            fprintf(stderr, "wimapply failed with status %d\n", WEXITSTATUS(status));
